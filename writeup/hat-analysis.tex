...
\textbf{Read Atomic (RA)} isolation.  Under RA, once some of the
...
cut isolation, RA prevents Read Skew anomalies (Berenson et al.'s A5A)
...
derived data. In the example below, under RA, because $T_2$ has read
...
% RA can be achieved via multi-versioning. If clients attach vector
...
% implemented via client-side caching). However, RA is achievable
...
maintain limited criteria like foreign key constraints (via RA). We do
...
Skew. Their phenomena prevention requirements mean that these
...
Writes (MW)\\\hline Sticky & Read Your Writes (RYW), PRAM,
...
\caption{Summary of highly available, sticky available, and
  ...
  recency guarantees\linearizable.}\vspace{-2em}
...
\begin{tikzpicture}[scale=0.8]
 ...
 \node[draw=none,fill=none] (ra) at (0, 1.475) {RA};
...
RA is non-obvious. RA dictates that writes to
...
finish. With RA, there is no recency
