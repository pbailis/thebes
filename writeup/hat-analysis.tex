...
databases, yet they cannot achieve all the traditional semantics. In
this section, we delineate ACID, distributed consistency, and session
...
your writes, PRAM and causal consistency) and the properties which
cannot be provided in a HAT system (those preventing Lost Update and
Write Skew, or with recency).  We present a full summary of these
results in Section~\ref{sec:hat-summary}.
...
systems. We offer proof-of-concept highly available algorithms, but
our goal is \textit{not} to provide optimal or even efficient
implementations. In our examples, we exclusively consider read and
write operations. We denote a write of value $v$ to data item $d$ as
$w_d(v)$ and a read from data item $d$ returning $v$ as $r_d(v)$ and
assume that all data items have the null value, $\bot$, at database
initialization. Unless otherwise specified, all transactions in the
examples commit.

\subsubsection{ACID Isolation Guarantees}

To begin, \textbf{Read Uncommitted} isolation is captured by Adya as
$PL-1$, requiring that each transaction's writes are ordered
consistently according to a single total order on transactions. This
prohibits Adya's phenomenon $G0$, also called ``Dirty
Writes''~\cite{adya}. We form a graph of transactions, with edges from
one transaction to another if the former writes to an object before
the latter writes to the same object. PL-1 prohibits cycles in this
graph, requiring some total order on writes. Accordingly, the final
...
versions. The implementations often provide recency and monotonicity
properties beyond the simple meaning from the name, which is what is
...
behavior allowed. In this paper, we reserve the use of ``Repeatable
Read'' to refer to these definitions, which we will soon show are not
HAT-compliant.
...
A stronger achievable property requires consistent cuts to span both a
...
achievable in HAT systems.

To achieve TA, clients buffer their writes until commit, sending the
final write to each data item to each respective server. Along with
each write, clients attach a the transaction ID and a list of all data
items written to in the transaction, $L_v$. Upon receiving a new write
to item $d$, a server places the write into a \textit{pending} queue
and notifies all other servers responsible for a data item in $L_v$
that it has received a write to $d$ with the respective transaction
ID. Once a server receives acknowledgments with appropriate
transaction ID for all items in $L_v$, it moves the write from
\textit{pending} to the \textit{stable} database (which applies the
writes according to highest-ID-wins). Accordingly, all writes that
should be made atomic with a write in \textit{stable} are guaranteed
to be present on their respective servers, either in \textit{pending}
or \textit{stable}. In the presence of internal aborts, writes may
require 2 RTTs: one to check constraints and one to execute the above.
...
performs a read from a data item, it sends the server its current
value for the item. If the value is empty, the server reads from
\texttt{stable}. If the value is non-empty, the server either returns
the matching value from \texttt{pending} or a higher-ID value from
\texttt{pending}---along with $L_v$ for the write. When the client
receives a response, it updates its local vector with
$L_v$. Accordingly, even if transaction's writes to two keys $x$ and
$y$ are present on the $x$ server's \textit{stable} and the $y$
server's \textit{pending}, a client's reads from $x$ then $y$ will
return a transactionally atomic pair of values because the $y$ server
will know to check pending.

This implementation is entirely masterless and both reads and writes
do not block for coordination. One key property is that we do not, in
general, dictate when writes become visible. Accordingly, it is highly
...
transaction values are \textit{pending} within a group of servers,
values can be made \textit{stable} (as opposed to once they are
...
transactions, other than that there exists some arbitrary ordering on
them (via Read Uncommitted). In the distributed systems literature,
many useful \textit{safety} guarantees span multiple,
non-transactional operations. In particular, \textit{session
  guarantees} are used to describe guarantees across transactions
within a given \textit{session}, ``an abstraction for the sequence
of...operations performed during the execution of an
...
Repeatable Read (defined by to Gray, Berenson et al., and Adya) and
...
range of isolation levels are achievable in HAT systems, including
...
compatible models (e.g., the diagram depicts 96 possible HAT
