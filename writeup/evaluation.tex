...
and MAV (session \textit{PL-2L}~\cite{adya}) but are unavailable in the
...
these guarantees matter in a representative transactional application
based on the TPC-C benchmark~\cite{tpcc}.
...
Section~\ref{sec:ta}. (hereafter, \texttt{MAV}). It also supports
...
writes in MAV are sent to a write-ahead log, which is synchronously
flushed to disk.
...
overhead). This reflects our \texttt{MAV} algorithm's metadata requirements,
