...
analysis on RA (algorithm from Section~\ref{sec:ta}, described in
greater detail
in Appendix B
...
RA's relative throughput decreased: every YCSB \texttt{put} operation
...
to Eventual and RC, causing RA throughput to peak at around half of
Eventual. With in-memory persistence (i.e., no LevelDB or WAL),
\texttt{RA} throughput was within 20\% of \texttt{eventual}.
...
overhead). This reflects our RA algorithm's metadata requirements,
