...
compared to Eventual and RC, causing TA throughput to peak at around half
of Eventual. With an in-memory persistence (i.e., no LevelDB or WAL),
...
is within $33\%$, and Eventual throughput decreases by $288.8\%$
compared to all reads and TA is within $33\%$ of eventual. At $99.8\%$
reads, TA incurs a $7\%$ overhead ($5.8\%$ for in-memory storage).
